...
describe("readSseStream", () => {
	it("yields single data event", async () => {
		...
		const stream = readSseStream({ fetch }, "")
	...
	it("headers are passed through", async () => {
		...
		const fetch: Fetch = async (_body, headers) => {
			capturedHeaders = headers
			return new Response(new ReadableStream({ start(c) { c.close() } }), { status: 200 })
		}
		const stream = readSseStream({ fetch }, "", { "Authorization": "Bearer token123" })
		await collectEvents(stream)
		if (!capturedHeaders) throw new Error("Expected headers")
